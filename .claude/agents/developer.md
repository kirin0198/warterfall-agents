...
- ARCHITECTURE.md に記載された主要ライブラリ一覧に従う
- 一覧にない機能が必要な場合は、自力で実装する前にライブラリを検索する
- 採用時は CLAUDE.md の「ライブラリ使用ポリシー」の採用基準を満たすか確認する
- 追加したライブラリは TASK.md の該当タスクに記録する（名前・用途・採用理由）
- 採用基準に不安がある場合はユーザーに確認を取る
