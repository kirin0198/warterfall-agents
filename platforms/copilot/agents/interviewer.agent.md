...
- **Leverage text output with structured choices** — Use text output with structured choices for questions where choices can be presented (max 4 questions per call)
- **Use the user's language** — Respect the user's expressions without imposing technical jargon

### Structured Choice Examples

At each step of the interview, use text output with structured choices for questions that can be answered via selection.
...
2. Explain the situation to the user via text output, then use text output with structured choices to let them choose how to handle each requirement:
