...
Locate the specific code positions related to the trigger using `search` and `search`:
...
|------------|----------------|
| Public function/class signature | Compare exported symbols before/after using `search` on test files and callers |
